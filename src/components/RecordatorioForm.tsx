...
export default function RecordatorioForm({ onSaved }: Props) {
  ...
  const [selectedPagoId, setSelectedPagoId] = useState<number | null>(
    pagos[0]?.id_pago ?? null,
  )
  ...
  const selectedPagoTitle = useMemo(
    () => pagos.find((p) => p.id_pago === selectedPagoId)?.titulo ?? null,
    [pagos, selectedPagoId],
  )
          ...
          <FlatList
            ...
            renderItem={({ item }: any) => (
              <Pressable
                ...
                onPress={() => {
                  setSelectedPagoId(item.id_pago)
        ...
        <DateTimePicker
          ...
          onChange={(e, selected) => {
            ...
            if (selected) {
              ...
              const iso = `${y}-${m}-${d}`
              setFecha(iso)
              console.log("RecordatorioForm: fecha seleccionada ->", {
                iso,
                selectedLocal: selected,
                tzOffsetMin: selected.getTimezoneOffset(),
              })
            }
          }}
        />
        ...
        <DateTimePicker
          ...
          onChange={(e, selected) => {
            ...
            if (selected) {
              ...
              setHora(`${hh}:${mm}`)
              console.log("RecordatorioForm: hora seleccionada ->", {
                hh,
                mm,
                hora: `${hh}:${mm}`,
              })
